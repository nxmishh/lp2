...
app.post("/musicd/singer", async (req, res) => {
  ...
  let result = await Songdetail.find({ music_director: musicd,singer: singer},);
  res.send(result);
});

app.get("/abc/:songname", async (req, res) => {
  let songname = req.params.songname;
  let result = await Songdetail.deleteMany({ song_name: songname });
  res.send(result);
});
